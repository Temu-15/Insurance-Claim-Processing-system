...
const ClaimDetailPage = () => {
  ...
  if (loading) {
    return (
      <main className={`flex min-h-screen ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-800'}`}>
        <AppSidebar />
        <div className="flex-1 p-6 md:p-8 flex justify-center items-center">
          <div className="flex flex-col items-center">
            ...
            <p className={`text-lg ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>Loading claim details...</p>
          </div>
        </div>
      </main>
    );
  ...
  if (claim === undefined) {
    return (
      <main className={`flex min-h-screen ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-800'}`}>
        <AppSidebar />
        <div className="flex-1 p-6 md:p-8 flex justify-center items-center">
          <div className={`max-w-md p-6 rounded-lg shadow-lg ${theme === 'dark' ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'}`}>
            <div className="flex items-center mb-4">
              <FiAlertCircle className={`w-8 h-8 mr-3 ${theme === 'dark' ? 'text-red-400' : 'text-red-500'}`} />
              <h2 className="text-xl font-bold">Claim Not Found</h2>
            </div>
            <p className={`mb-6 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
              We couldn't find the claim you're looking for. It may have been removed or the claim number is incorrect.
            </p>
            <button
        ...
        <div className="flex-1 p-6 md:p-8">
          ...
          <div className="flex items-center mb-6">
            <motion.button 
              ...
              whileTap={{ scale: 0.95 }}
              onClick={() => navigate('/user/claims')}
              className={`mr-4 p-2 rounded-full ${theme === 'dark' ? 'hover:bg-gray-800' : 'hover:bg-gray-200'} transition-colors`}
              aria-label="Go back"
            >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <motion.div 
              ...
              className={`lg:col-span-3 rounded-xl shadow-md overflow-hidden ${theme === 'dark' ? 'bg-gray-800 border border-gray-700' : 'bg-white border border-gray-200'}`}
            >
              <div className={`p-6 md:p-8 flex flex-col md:flex-row items-start md:items-center justify-between border-b ${theme === 'dark' ? 'border-gray-700' : 'border-gray-200'}`}>
                <div className="flex items-center">
                  <div className={`p-3 rounded-full mr-4 ${theme === 'dark' ? 'bg-blue-900' : 'bg-blue-100'}`}>
                    <FiFileText className={`w-6 h-6 ${theme === 'dark' ? 'text-blue-300' : 'text-blue-600'}`} />
                  </div>
                  <div>
                ...
                <div className={`mt-4 md:mt-0 p-4 rounded-lg ${theme === 'dark' ? 'bg-gray-700' : 'bg-gray-50'}`}>
                  <div className={`text-sm ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>Amount Requested</div>
              ...
              <div className="p-6 md:p-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div>
                    <h3 className={`text-lg font-semibold mb-4 flex items-center ${theme === 'dark' ? 'text-gray-200' : 'text-gray-700'}`}>
                    ...
                    <div className={`space-y-4 p-4 rounded-lg ${theme === 'dark' ? 'bg-gray-700' : 'bg-gray-50'}`}>
                      <div className="flex flex-col">
                        ...
                        <span className={`mt-1 ${theme === 'dark' ? 'text-gray-200' : 'text-gray-800'}`}>
                          {claim.policyId}
                        </span>
                      </div>
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className={`mt-1 flex items-center ${theme === 'dark' ? 'text-gray-200' : 'text-gray-800'}`}>
                  ...
                  <div>
                    ...
                    <div className={`p-4 rounded-lg min-h-[200px] ${theme === 'dark' ? 'bg-gray-700' : 'bg-gray-50'}`}>
              ...
              {/* Status Timeline */}
              <div className={`p-6 md:p-8 border-t ${theme === 'dark' ? 'border-gray-700' : 'border-gray-200'}`}>
                ...
                <div className="relative">
                  ...
                  <div className="space-y-6">
                    <div className="relative flex items-start">
                      <div className={`absolute left-0 mt-1 w-8 h-8 rounded-full flex items-center justify-center ${
                        theme === 'dark' ? 'bg-blue-900 text-blue-300' : 'bg-blue-100 text-blue-600'
                      }`}>
                        <FiFileText className="w-4 h-4" />
                      </div>
                      <div className="ml-12">
                        <h4 className={`text-md font-medium ${theme === 'dark' ? 'text-gray-200' : 'text-gray-800'}`}>
                          Claim Submitted
                        </h4>
                      ...
                      <div className="relative flex items-start">
                        <div className={`absolute left-0 mt-1 w-8 h-8 rounded-full flex items-center justify-center ${
                          claim.status.toLowerCase() === 'approved'
                            ...
                            : theme === 'dark' ? 'bg-red-900 text-red-300' : 'bg-red-100 text-red-600'
                        }`}>
                          {claim.status.toLowerCase() === 'approved' 
