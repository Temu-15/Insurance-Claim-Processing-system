    ...
    return (
      <div className={`min-h-screen ${getBgColor()} flex items-center justify-center`}>
        <div className="text-center">
          <div className={`animate-spin rounded-full h-12 w-12 border-b-2 ${theme === "dark" ? "border-brand-400" : "border-brand-500"} mx-auto`}></div>
          <p className={`mt-4 ${getSubTextColor()}`}>Loading policy details...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className={`min-h-screen ${getBgColor()} flex items-center justify-center`}>
        <div className={`${getCardBgColor()} p-8 rounded-lg shadow-md max-w-md w-full`}>
          ...
          <h2 className={`text-xl font-semibold text-center ${getTextColor()} mb-4`}>
            Error
          </h2>
          ...
          <div className="mt-6 text-center">
            <Link
              to="/user/dashboard"
              className={`${theme === "dark" ? "text-brand-400" : "text-brand-500"} hover:${theme === "dark" ? "text-brand-300" : "text-brand-600"} font-medium`}
            >
              Return to Dashboard
            </Link>
          </div>
    ...
    <div className={`min-h-screen ${getBgColor()} py-12 px-4 sm:px-6 lg:px-8`}>
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          <Link
            to="/user/policies"
            className={`inline-flex items-center ${theme === "dark" ? "text-brand-400" : "text-brand-500"} hover:${theme === "dark" ? "text-brand-300" : "text-brand-600"}`}
          >
        ...
        <div className={`${getCardBgColor()} shadow-xl rounded-lg overflow-hidden ${getBorderColor()} border`}>
          {/* Policy Header */}
          <div className={`${theme === "dark" ? "bg-brand-800" : "bg-brand-600"} text-white px-6 py-4`}>
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className={`${getHeaderBgColor()} p-4 rounded-lg ${getBorderColor()} border`}>
            ...
            <div className="mt-6">
              ...
              <div className={`${getHeaderBgColor()} p-4 rounded-lg ${getBorderColor()} border`}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <p className={`text-sm ${theme === "dark" ? "text-gray-400" : "text-gray-500"}`}>Policy ID</p>
                    <p className={`${getTextColor()}`}>{policy.policyId}</p>
                  </div>
                  <div>
                    <p className={`text-sm ${theme === "dark" ? "text-gray-400" : "text-gray-500"}`}>Policy Created</p>
            ...
            <div className="mt-8 flex justify-center space-x-4">
              <Link
                ...
                className={`inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white ${theme === "dark" ? "bg-brand-600 hover:bg-brand-500" : "bg-brand-500 hover:bg-brand-600"} focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-500`}
              >
                File a Claim
              </Link>
              <Link
                to={`/policies/${policy.policyId}/documents`}
                className={`inline-flex items-center px-4 py-2 border ${theme === "dark" ? "border-gray-600 bg-gray-700 text-gray-200 hover:bg-gray-600" : "border-gray-300 bg-white text-gray-700 hover:bg-gray-50"} rounded-md shadow-sm text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-brand-500`}
              >
                View Documents
              </Link>
            </div>
